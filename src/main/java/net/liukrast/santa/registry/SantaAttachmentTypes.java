...
public class SantaAttachmentTypes {
    ...
    public static final Supplier<AttachmentType<Integer>> TRUST = REGISTER.register(
            "trust", () -> AttachmentType.builder(() -> 0)
                    .serialize(Codec.intRange(-1_000_000_000, 1_000_000_000))
                    .sync(ByteBufCodecs.INT).build()
    );

    public static final Supplier<AttachmentType<Optional<SantaTradeProgress>>> SANTA_TRADE_PROGRESS = REGISTER.register(
            "santa_trade_progress", () -> AttachmentType.<Optional<SantaTradeProgress>>builder(Optional::empty)
                    .serialize(SantaTradeProgress.OPTIONAL_CODEC)
                    .sync(SantaTradeProgress.OPTIONAL_STREAM_CODEC)
                    .build()
    );
